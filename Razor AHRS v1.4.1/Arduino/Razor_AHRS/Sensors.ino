...
void Accel_Init()
{
  ...
  WIRE_SEND(0x08);  // Measurement mode, no sleep/wake-up/auto-sleep & (activity and inactivity functions happen together)
  Wire.endTransmission();
  delay(5);
  Wire.beginTransmission(ACCEL_ADDRESS);
  WIRE_SEND(0x31);  // Data format register
  WIRE_SEND(0x08);  // Set to full resolution (4mg/LSB)....+/-2G; 4 wire SPI, 
  Wire.endTransmission();
  delay(5);
...
void Gyro_Init()
{
  ...
  Wire.beginTransmission(GYRO_ADDRESS);
  WIRE_SEND(0x16);
  WIRE_SEND(0x1B);  // DLPF_CFG = 3, FS_SEL = 3 42 Hz low pass filter
  //WIRE_SEND(0x19);  // DLPF_CFG = 1, FS_SEL = 3 Modified ABY for 188 Hz
  Wire.endTransmission();
  delay(5);
  ...
  Wire.beginTransmission(GYRO_ADDRESS);
  WIRE_SEND(0x15);
  WIRE_SEND(0x13);                      //  SMPLRT_DIV = 10 (50Hz)...19 so that we can divide by 20 Modified ABY
  Wire.endTransmission();
  delay(5);
